...
class CV_Tool:
    def __init__(self, visualize=False, method=None, permute=False, seed=0, random_train=False, random_voxel=False):
    ...
    def transform_data(self, X_array, Y_array, leave_out_sub):
        ...
        if self.method is None:
            ...
            if self.random_train:
                X_test = np.random.rand(*X_array[leave_out_sub].shape)
                X_test = procrustes(X_test, Template)
            else:
